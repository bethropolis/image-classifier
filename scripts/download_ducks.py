#!/usr/bin/env python3
"""Download duck images from random-d.uk into train/val/test class folders.

API: https://random-d.uk/api/random
Response: {"url": "https://random-d.uk/api/...", "message": "..."}
"""
...
USER_AGENT = "image-classifier-duck-downloader/1.0"

LOGGER = logging.getLogger("download_ducks")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Download duck images into data splits")
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            if attempt >= max_retries:
                raise RuntimeError(f"Failed to fetch duck URL after {max_retries} attempts") from exc
...
def make_destination(split_dir: Path, image_url: str, index: int) -> Path:
    ...
    return split_dir / f"duck_{index:05d}_{digest}{ext}"
...
def main() -> None:
    ...
    dirs = {
        "train": args.data_dir / "train" / "ducks",
        "val":   args.data_dir / "val"   / "ducks",
        "test":  args.data_dir / "test"  / "ducks",
    }
    ...
    LOGGER.info("Finished. %d new duck images downloaded in total.", total)
